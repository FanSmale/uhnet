...
# """
#
# # main_data_dir = "C:/Users/hp/Desktop/UTNet01/data/SEGSimulateData/"
# '''
...
#         data_set_ = scipy.io.loadmat(data_path + '/georec' + str(i + 1))['Rec']
...
通过.mat文件读取数据
"""
# D:\Zhang-Xingyi\Data for FWI\SimulateData\test_data\georec_test
def SEGSimulate(base_dir, start, instance_size, in_channels, split, data_dsp_blk, label_dsp_blk, model_dim):
    if split == 'train':
        # 加载.mat格式文件
        ...
        # 加载.npy格式文件
        # _data_set = np.load(os.path.join(base_dir, 'seismic.npy'))
        # _label_set = np.load(os.path.join(base_dir, 'vmodel.npy'))
        # base_dir = 'D:/Zhong-Xiyan/TransUNet-FWI01/datasets/SEGSimulateData'
        # _data_set = np.load(os.path.join(base_dir, 'seismic.npy'))
        # _label_set = np.load(os.path.join(base_dir, 'vmodel.npy'))

        base_dir = 'E:/fwi/data/'
        _data_set = np.load(os.path.join(base_dir, 'trainData_1600groups_29channels_400x301_georecData.npy'))
        _label_set = np.load(os.path.join(base_dir, 'trainData_1600groups_1channel_201x301_vmodelData.npy'))
        return _data_set[:instance_size], _label_set[0][:instance_size]

    # if split == 'train':
    #     # 加载.mat格式文件
    #     data_path = os.path.join(base_dir, 'train_data/seismic')
    #     label_path = os.path.join(base_dir, 'train_data/vmodel')
    #
    #     # 加载.npy格式文件
    #     # _data_set = np.load(os.path.join(base_dir, 'seismic.npy'))
    #     # _label_set = np.load(os.path.join(base_dir, 'vmodel.npy'))
    ...
    #     # return _data_set[:instance_size], _label_set[0][:instance_size]
        ...
        data_path = "E:/fwi/data/SEGSimulate/test_data/seismic"
        label_path = "E:/fwi/data/SEGSimulate/test_data/vmodel"


    for i in range(start, start + instance_size):
        data_set_ = scipy.io.loadmat(data_path+'/seismic'+str(i+1))['data']
        label_set_ = scipy.io.loadmat(label_path+'/vmodel'+str(i+1))['data']
        if (i+1) % 50 == 0 : print(data_path+'/seismic'+str(i+1))
        # Change the dimention [h, w, c] --> [c, h, w]
